...
const $all = (query: string): HTMLElement[] => Array.prototype.slice.call(document.querySelectorAll(query))

// Load all canvasses and ctxes
const canvasses = $all('.ctx') as HTMLCanvasElement[]
const ctxes: CanvasRenderingContext2D[] = []

const getCtxes = () => {
  const canvasses = $all('.ctx') as HTMLCanvasElement[]
  return canvasses.map(canvasEl => canvasEl.getContext('2d'))
}
...
const isolation = $('.isolation') as HTMLCanvasElement
const isolationCtx = isolation.getContext('2d')

const shop = $('.shop') as HTMLCanvasElement
const shopCtx = shop.getContext('2d')
...
// Create Population
const population = new Population(1500, ctxes)
...
const notInfectedData: Partial<Plotly.PlotData> = {
  x: [],
  y: [],
  name: 'Not Infected',
  type: 'scatter',
...
const reset = () => {
  ...
  ([
    ...
    deadData,
    maxHealthCare
  ]).forEach((data: Partial<Plotly.PlotData>) => {
    data.x = []
    data.y = []
  })
}
...
const renderFrame = () => {
  // Clear canvasses
  ($all('canvas') as HTMLCanvasElement[]).map(canvasEl => canvasEl.getContext('2d')).forEach(ctx => {
    ctx.clearRect(0, 0, ctx.canvas.width, ctx.canvas.height)
  })

  // Render Population
  console.log(
    `Rendering population took: ${
      population.render()
    }ms`
  )
...
const changePopulationSize = (el: HTMLInputElement) => {
  let value = parseFloat(el.value)
  population.constructor(value, getCtxes())
}

const changeNumberOfCommunities = (el: HTMLInputElement) => {
  let value = parseFloat(el.value)
...
const changeSpreadRange = (el: HTMLInputElement) => {
  let value = parseFloat(el.value)
  Human.rangeOfSpread = value
}

const changeSpreadProbability = (el: HTMLInputElement) => {
  let value = parseFloat(el.value)
  Human.spreadProbability = value
}

const changeSocialDistancingRange = (el: HTMLInputElement) => {
  let value = parseFloat(el.value)
  Human.socialDistancingRange = value
}

const changeSocialDistancingRatio = (el: HTMLInputElement) => {
  let value = parseFloat(el.value)
...
const changeTravelProbability = (el: HTMLInputElement) => {
  let value = parseFloat(el.value)
  population.travelProbability = value
}

const changeShopProbability = (el: HTMLInputElement) => {
  let value = parseFloat(el.value)
  population.shopProbability = value
}

const changeIsolationProbability = (el: HTMLInputElement) => {
  let value = parseFloat(el.value)
  population.isolationProbability = value
}

const enableIsolation = (enable: boolean) => {
  population.isolationActive = enable
}

const enableSocialDistancing = (enable: boolean) => {
  if (enable) {
    population.enableSocialDistancing(population.socialDistancingRatio)
  } else {
...
const enableLockdown = (enable: boolean) => {
  population.lockdownActive = enable
}

const closeShop = (enable: boolean) => {
  population.shopClosed = enable
}

const changeIsolationThresholdRatio = (el: HTMLInputElement) => {
  let value = parseFloat(el.value)
  Population.isolationThresholdRatio = value
  population.calculateThresholds()
}

const changeSocialDistancingThresholdRatio = (el: HTMLInputElement) => {
  let value = parseFloat(el.value)
  Population.socialDistancingThresholdRatio = value
  population.calculateThresholds()
}

const changeLockdownThresholdRatio = (el: HTMLInputElement) => {
  let value = parseFloat(el.value)
  Population.lockdownThresholdRatio = value
  population.calculateThresholds()
}

const changeShopCloseThresholdRatio = (el: HTMLInputElement) => {
  let value = parseFloat(el.value)
...
const inputSubmits = $all('div > input')
...
for (let i = 0; i < inputSubmits.length; i++) {
  if (inputSubmits[i].parentElement.querySelector('button') != null) {
    if (inputSubmits[i].parentElement.querySelector('button').innerText == 'Submit') {
      inputSubmits[i].addEventListener('input', () => {
        inputSubmits[i].parentElement.querySelector('button').style.border = '2px solid #cc5'
      })
      inputSubmits[i].addEventListener('keydown', e => {
        if (e.key == 'Enter') {
          inputSubmits[i].parentElement.querySelector('button').click()
        }
      })
      inputSubmits[i].parentElement.querySelector('button').addEventListener('click', () => {
...
for (let numberInput of $all('input[type="number"]')) {
  numberInput.addEventListener('mousewheel', e => {
    numberInput.blur()
  })
}
